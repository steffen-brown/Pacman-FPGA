...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE inBuffer[2];
	int ret = XSpi_Transfer(&SpiInstance, outBuffer, inBuffer, 2);

	//if return code != 0 print an error
	if(ret != 0) {
		xil_printf("SPI Transfer in MAXreg_wr failed.");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(int i = 1; i <= nbytes; i++) {
		outBuffer[i] = data[i - 1];
	}

	//read return code from SPI peripheral 
	BYTE inBuffer[64];
	int ret = XSpi_Transfer(&SpiInstance, outBuffer, inBuffer, nbytes + 1);

	//if return code != 0 print an error
	if(ret != 0) {
		xil_printf("SPI Transfer in MAXbytes_wr failed.");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//if return code != 0 print an error
	if(ret != 0) {
		xil_printf("SPI Transfer in MAXreg_rd failed.");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//if return code != 0 print an error
	if(ret != 0) {
		xil_printf("SPI Transfer in MAXbytes_rd failed.");
	}
