...
int pinky_exit_ghost_house(uint32_t g_x, uint32_t g_y, uint32_t* g_dir, uint32_t* g_mv) {
    // Define the target pixel for Pinky to move up to (exit the ghost house)
    ...
    // Check if Pinky needs to move up
    if (g_y > target_exit_pixel_y) {
        *g_dir = 3; // Move Up
        *g_mv = 1;  // Enable movement
    }
    // Check if Pinky has reached the exit row but not yet the left wall
    else if (g_y <= target_exit_pixel_y && g_x > left_wall_pixel_x) {
        *g_dir = 2; // Move Left
        *g_mv = 1;  // Enable movement
    }
    // Once Pinky has reached the wall
    else {
        return 0;
    }
    return 1;
