...
int clyde_exit_ghost_house(uint32_t g_x, uint32_t g_y, uint32_t* g_dir, uint32_t* g_mv) {
    ...
    int target_left_pixel_x = 13 * CELL_SIZE + OFFSET_X + 4;   // Column 13 in pixels (left of Clyde's ghost house position)
    int target_exit_pixel_y = 11 * CELL_SIZE + OFFSET_Y - 4;   // Row 11 in pixels (fully clear the ghost house)
    int right_wall_pixel_x = 13 * CELL_SIZE + OFFSET_X - 1;    // Column 13 in pixels (right wall for alignment)

    // Step 1: Move Left to align with the ghost house exit
    if (g_x > target_left_pixel_x) {
        *g_dir = 2; // Move Left
        *g_mv = 1;  // Enable movement
        return 1;   // Exit not completed
    }
    // Step 2: Move Up to clear the ghost house vertically
    else if (g_y > target_exit_pixel_y) {
        *g_dir = 3; // Move Up
        *g_mv = 1;  // Enable movement
        return 1;   // Exit not completed
    }
    // Step 3: Move Right along the gameplay grid to start chasing Pac-Man
    else if (g_x < right_wall_pixel_x) {
        *g_dir = 0; // Move Right
        *g_mv = 1;  // Enable movement
        return 1;   // Exit not completed
    }
